...
async function findClaudeCliPath() {
    ...
    if (process.platform !== 'win32') {
        try {
            const { stdout } = await execAsync('source ~/.zshrc 2>/dev/null || source ~/.bashrc 2>/dev/null || true; which claude', {
                shell: '/bin/bash'
            });
        ...
        if (await hasClaudeCodeCLI()) {
            ...
            if (preferredMethod === 'cli') {
                ...
                if (userPath) {
                    ...
                    throw new Error('Claude CLI not found and no path configured. Run "which claude" in terminal to find the path.');
                }
            }
        }
    }
    ...
    if (preferredMethod === 'api' || preferredMethod === 'auto') {
        ...
        try {
            ...
            if (cliNotFound && error.message.includes('ANTHROPIC_API_KEY')) {
                throw new Error('Claude CLI not found and no API key configured. Either configure CLI path in settings or set ANTHROPIC_API_KEY.');
            }
